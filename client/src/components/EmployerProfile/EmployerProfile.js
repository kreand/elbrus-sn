...
const EmployerProfile = () => {
  ...
  const [emp, setEmp] = useState({
    ...employer,
    allReviews: employer.allReviews.map(rev => ({
      ...rev,
  ...
  const sortByDate = () => {
    if (btns.date) {
      setEmp(prev => ({
        ...prev,
        allReviews: { ...prev }.allReviews.sort((a, b) => a.date - b.date),
      }));
    } else {
      setEmp(prev => ({
        ...prev,
        allReviews: { ...prev }.allReviews.sort((a, b) => b.date - a.date),
      }));
    }
    setBtns(prev => {
  ...
  const sortByRating = () => {
    if (btns.rating) {
      setEmp(prev => ({
        ...prev,
        allReviews: { ...prev }.allReviews.sort((a, b) => a.rating - b.rating),
      }));
    } else {
      setEmp(prev => ({
        ...prev,
        allReviews: { ...prev }.allReviews.sort((a, b) => b.rating - a.rating),
      }));
    }
    setBtns(prev => {
        ...
        <div className={style.buttonContainer}>
          ...
          <div style={{ paddingBottom: 0 }}>
            ...
            <Link to="#" onClick={sortByDate} className={style.text}>
              {btns.date ? <DownOutlined /> : <UpOutlined />}
              {' Дата'}
            </Link>
            <Link
              to="#"
            ...
            >
              {btns.rating ? <DownOutlined /> : <UpOutlined />}
              {' Рейтинг'}
            </Link>
          </div>
        </div>
        <EmployerReviewsList
          allReviews={emp.allReviews.slice((page - 1) * 10, page * 10)}
          employerId={emp._id}
        />
        <PaginationComponent
          totalPages={emp.allReviews.length}
